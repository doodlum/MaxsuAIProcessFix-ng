...
#define PI acos(-1)
	...
	struct P_Vec
	{
		...
		P_Vec(float x, float y, float dir) :x{ x }, y{ y }, dir{ dir } {}

		float GetHeadingAngle(P_Vec const& other, bool b_unsign)
		{
			float heading = std::atan2(other.x - this->x, other.y - this->y) * 180 / PI - this->dir;

			if (heading < -180) heading += 360;

			if (heading > 180) heading -= 360;

			return b_unsign ? abs(heading) : heading;
		}
	};
	...
	std::optional<RE::NiPoint3>	TESObjectREFR::FindNearestVertex(const float minimum_offset)
	{
		...
		for (auto& navMesh : navMeshes.navMeshes) 
		{
			...
			{
				auto linearDistance = this->GetPosition().GetDistance3D(vertex.location);
	...
	float TESObjectREFR::GetDistanceTo2DPoint(float a_x, float a_y) const
	{
		return sqrt(pow(this->GetPositionX() - a_x, 2) + pow(this->GetPositionY() - a_y, 2));
	}



	float TESObjectREFR::GetDistanceTo3DPoint(const NiPoint3 &a_pos) const
	{

		return sqrt(pow(this->GetPositionX() - a_pos.x, 2) + pow(this->GetPositionY() - a_pos.y, 2) + pow(this->GetPositionZ() - a_pos.z, 2));
	}
	...
	float TESObjectREFR::GetHeadingAngle(TESObjectREFR* targetref, bool b_unsign) const
	{
		P_Vec thispos(this->GetPositionX(), this->GetPositionY(), this->GetAngleZ() * 180 / PI);

		P_Vec targetpos(targetref->GetPositionX(), targetref->GetPositionY(), 0);

		return thispos.GetHeadingAngle(targetpos, b_unsign);
	...
	bool TESObjectREFR::MoveToNearestNavmesh(const float minimum_offset)
	{
		...
		MoveTo_Impl(this->AsReference()->CreateRefHandle(), this->GetParentCell(), this->GetWorldspace(), std::move(*nearestVertex), this->GetAngle());
	...
	bool TESObjectREFR::MoveToRef(TESObjectREFR* a_target)
	{

		assert(a_target);
		auto handle = this->CreateRefHandle();
		MoveTo_Impl(handle, a_target->GetParentCell(), a_target->GetWorldspace(), a_target->GetPosition(), a_target->GetAngle());
		return true;
	...
	void TESObjectREFR::UnkObjRefrFunc01()
	{
		using func_t = decltype(&TESObjectREFR::UnkObjRefrFunc01);
		REL::Offset<func_t> func(Offset::TESObjectREFR::UnkObjRefFunc01);
		return func(this);
	}
